...
Os dois comandos públicos (`POST /v1/entries` e `POST /v1/entries/{entry_id}/reversals`) declaram no OpenAPI gerado o header obrigatório `Idempotency-Key`, sucesso `200` (grpc-gateway não traduz a resposta unária em `201`, então o contrato documenta o status real) e respostas de validação, autenticação/autorização e conflito `409`; o estorno também documenta o `404` anti-enumeração. Como o `entry_id` do estorno já vem no path, esse comando não possui body e clientes não precisam enviar um objeto `{}` vazio. O enforcement, o mapeamento HTTP e a idempotência transacional pertencem aos tickets de runtime, esta fundação entrega o contrato, não um mock de sucesso.
