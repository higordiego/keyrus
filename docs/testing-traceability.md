...
| Testes multi-tenant executados | T02 cobre identidade, isolamento e anti-enumeração; T04/T06 cobrem consolidação assíncrona e resultados financeiros exatos; regras financeiras de estorno/duplicidade concorrente permanecem fora do manifesto |
| Script e relatório k6, **duas evidências distintas, não a mesma prova** (fixup rnf03-rate-limit-vs-capacity) | (1) **Via o Edge**: `test/k6/load.js`, 100 VUs concorrentes, cada um executando a jornada completa (criar, consultar, listar, saldo diário, estornar) contra as 5 rotas públicas via KrakenD. 100% de sucesso (23.015 checks), p95 = 89,3 ms, `http_req_failed` em 0%. Evidência em `evidence/reports/load-test-summary.json`. (2) **Capacidade do domínio isolado**: `test/k6/load-backend.js`, 50 VUs contra `ledger-api`/`consolidation-api` diretamente na rede Docker, sem passar pelo KrakenD. 100% de sucesso (4036/4036 checks), p95 = 6,2 ms (limiar do SLO é <200 ms), taxa de erro 0% (limiar é <1%). Evidência em `evidence/reports/load-test-backend-summary.json`. **O Ledger/Consolidado sozinhos sustentam o perfil de carga testado dentro do SLO com folga, com ou sem o Edge no caminho.** Histórico: `client_max_rate` do KrakenD começou em 20/IP (a evidência original desta linha mostrava 47% de sucesso/53% de HTTP 429 nesse limite, provando a política de rate limit por IP, não a capacidade do domínio) e foi deliberadamente elevado para 2.000/IP para viabilizar avaliação local com mais VUs (ver nota no `README.md`); no valor atual o Edge não é mais o fator limitante nos testes rodados. Continua registrado como risco conhecido para produção: um limite por IP penaliza coletivamente usuários atrás do mesmo NAT corporativo; um limite por identidade autenticada (`merchant_id`) em vez de IP não foi implementado neste ticket. |
